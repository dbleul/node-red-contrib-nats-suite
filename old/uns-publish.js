...
module.exports = function (RED) {
  function NatsPublishNode(config) {
    ...
    if (bufferSizeBytes > MAX_BUFFER_SIZE_BYTES) {
      node.warn(`[UNS PUBLISH] Buffer size ${bufferSizeBytes} bytes exceeds maximum of ${MAX_BUFFER_SIZE_BYTES} bytes (100 MB). Using maximum.`);
      bufferSizeBytes = MAX_BUFFER_SIZE_BYTES;
    } else if (bufferSizeBytes < MIN_BUFFER_SIZE_BYTES) {
      node.warn(`[UNS PUBLISH] Buffer size ${bufferSizeBytes} bytes is below minimum of ${MIN_BUFFER_SIZE_BYTES} bytes (1 KB). Using minimum.`);
    ...
    const getBufferFilePath = () => {
      ...
      if (!fs.existsSync(bufferDir)) {
        try {
          ...
          node.warn(`[UNS PUBLISH] Failed to create buffer directory: ${err.message}`);
          return null;
        }
      }
    ...
    const loadBuffer = () => {
      ...
      if (bufferPersistence === 'context' || bufferPersistence === 'both') {
        try {
          ...
          if (savedQueue && Array.isArray(savedQueue) && savedQueue.length > 0) {
            ...
            if (isDebug) {
              node.log(`[UNS PUBLISH] Loaded ${messageQueue.length} messages from Context Storage`);
            }
          }
        } catch (err) {
          node.warn(`[UNS PUBLISH] Failed to load buffer from Context Storage: ${err.message}`);
        }
      }
          ...
          if (filePath && fs.existsSync(filePath)) {
            ...
            if (saved.queue && Array.isArray(saved.queue) && saved.queue.length > 0) {
              ...
              if (loadedFromContext && bufferPersistence === 'both') {
                if (isDebug) {
                  node.log(`[UNS PUBLISH] Merging file buffer (${saved.queue.length} msgs) with context buffer (${messageQueue.length} msgs)`);
                }
              ...
              loadedFromFile = true;
              if (isDebug) {
                node.log(`[UNS PUBLISH] Loaded ${messageQueue.length} messages from File System`);
              }
            }
          }
        } catch (err) {
          node.warn(`[UNS PUBLISH] Failed to load buffer from File System: ${err.message}`);
        }
      }
      ...
      if ((loadedFromContext || loadedFromFile) && messageQueue.length > 0) {
        ...
        node.log(`[UNS PUBLISH] Restored ${messageQueue.length} buffered messages from ${bufferPersistence === 'both' ? 'Context + File' : bufferPersistence}`);
        ...
        setImmediate(() => {
          setTimeout(() => {
            if (this.config && this.config.connectionStatus === 'connected' && messageQueue.length > 0 && !isFlushing) {
              node.log(`[UNS PUBLISH] Connection already established, flushing ${messageQueue.length} restored messages...`);
              flushBuffer();
            }
    ...
    const saveBuffer = (force = false) => {
      ...
      if (messageQueue.length === 0 && force) {
        ...
        if (bufferPersistence === 'context' || bufferPersistence === 'both') {
          try {
            ...
            if (isDebug) {
              node.log(`[UNS PUBLISH] Cleared buffer from Context Storage`);
            }
          } catch (err) {
            node.warn(`[UNS PUBLISH] Failed to clear buffer from Context Storage: ${err.message}`);
          }
        }
        ...
        if (bufferPersistence === 'file' || bufferPersistence === 'both') {
          try {
            ...
            if (filePath && fs.existsSync(filePath)) {
              ...
              if (isDebug) {
                node.log(`[UNS PUBLISH] Cleared buffer file from File System`);
              }
            }
          } catch (err) {
            node.warn(`[UNS PUBLISH] Failed to clear buffer file from File System: ${err.message}`);
          }
        }
        return;
      }
      ...
      if (bufferPersistence === 'context' || bufferPersistence === 'both') {
        try {
          ...
          if (isDebug && force) {
            node.log(`[UNS PUBLISH] Saved ${messageQueue.length} messages to Context Storage`);
          }
        } catch (err) {
          node.warn(`[UNS PUBLISH] Failed to save buffer to Context Storage: ${err.message}`);
        }
      }
      ...
      if (bufferPersistence === 'file' || bufferPersistence === 'both') {
        try {
          ...
          if (filePath) {
            ...
            if (isDebug && force) {
              node.log(`[UNS PUBLISH] Saved ${messageQueue.length} messages to File System`);
            }
          }
        } catch (err) {
          node.warn(`[UNS PUBLISH] Failed to save buffer to File System: ${err.message}`);
        }
      }
    };
    ...
    if (enableBuffer && isDebug) {
      if (bufferSizeType === 'count') {
        node.log(`[UNS PUBLISH] Message buffering enabled: size=${bufferSize} messages, mode=${bufferMode}`);
      } else {
        node.log(`[UNS PUBLISH] Message buffering enabled: size=${bufferSizeBytes} bytes (${formatBytesToMB(bufferSizeBytes)} MB), mode=${bufferMode}`);
      }
    }
    ...
    if (enableBatch && isDebug) {
      node.log(`[UNS PUBLISH] Batch publishing enabled: size=${batchSize}, interval=${batchInterval}ms, mode=${batchMode}`);
    }
    ...
    if (enableAutoReply && isDebug) {
      node.log(`[UNS PUBLISH] Auto-reply handler enabled: timeout=${replyTimeout}ms`);
    }
    ...
    if (enableRateLimit && isDebug) {
      node.log(`[UNS PUBLISH] Rate limiting enabled: ${rateLimit} msg/${rateLimitWindow}ms, burst=${rateLimitBurst}, action=${rateLimitAction}`);
    }
    ...
    const refillTokens = () => {
      ...
      if (isDebug && tokensToAdd > 0) {
        node.log(`[UNS PUBLISH] Token bucket refilled: +${tokensToAdd.toFixed(2)} tokens (total: ${tokenBucket.toFixed(2)}/${rateLimit + rateLimitBurst})`);
      }
    };
      ...
      switch (status) {
        case 'connected':
          ...
          if (enableBuffer && messageQueue.length > 0) {
            node.log(`[UNS PUBLISH] Connection restored, flushing ${messageQueue.length} buffered messages...`);
      ...
      if (isFull) {
        ...
        switch (bufferMode) {
          case 'drop-oldest':
            ...
            if (isDebug) {
              if (bufferSizeType === 'count') {
                node.log(`[UNS PUBLISH] Buffer full (${messageQueue.length}/${bufferSize} messages), dropped oldest message`);
              } else {
                node.log(`[UNS PUBLISH] Buffer full (${formatBytesToMB(currentValue)}/${formatBytesToMB(bufferSizeBytes)} MB), dropped oldest message`);
              }
            }
            break;
          case 'drop-newest':
            ...
            if (isDebug) {
              if (bufferSizeType === 'count') {
                node.log(`[UNS PUBLISH] Buffer full (${messageQueue.length}/${bufferSize} messages), dropped newest message`);
              } else {
                node.log(`[UNS PUBLISH] Buffer full (${formatBytesToMB(currentValue)}/${formatBytesToMB(bufferSizeBytes)} MB), dropped newest message`);
              }
            }
            return false; // Don't add new message
          case 'drop-on-full':
            if (bufferSizeType === 'count') {
              node.warn(`[UNS PUBLISH] Buffer full (${messageQueue.length}/${bufferSize} messages), message rejected`);
            } else {
              node.warn(`[UNS PUBLISH] Buffer full (${formatBytesToMB(currentValue)}/${formatBytesToMB(bufferSizeBytes)} MB), message rejected`);
            }
            return false; // Don't add new message
        }
      }
    ...
    const flushBuffer = async () => {
      ...
      if (isFlushing) {
        if (isDebug) {
          node.log(`[UNS PUBLISH] Flush already in progress, skipping...`);
        }
        return;
      }
      ...
      if (bufferSizeType === 'count') {
        node.log(`[UNS PUBLISH] Flushing ${flushCount} buffered messages...`);
      } else {
        node.log(`[UNS PUBLISH] Flushing ${flushCount} buffered messages (${formatBytesToMB(flushSize)} MB)...`);
      }
      ...
      if (node.config.connectionStatus !== 'connected') {
        ...
        invalidateBufferSizeCache();
        node.warn(`[UNS PUBLISH] Connection lost before flush, ${messageQueue.length} messages re-queued`);
        isFlushing = false;
        return;
      }
      ...
      const flushPromises = queueCopy.map(queuedMsg => 
        new Promise((resolve) => {
          try {
            ...
            node.error(`[UNS PUBLISH] Error flushing buffered message: ${err.message}`, queuedMsg);
      ...
      if (droppedMessages > 0) {
        node.warn(`[UNS PUBLISH] ${droppedMessages} messages were dropped due to buffer overflow`);
        droppedMessages = 0;
      }
      
      const remainingSize = bufferSizeType === 'size' ? calculateBufferSize() : 0;
      if (bufferSizeType === 'count') {
        node.log(`[UNS PUBLISH] Buffer flushed: ${successCount} sent, ${failCount} failed, ${messageQueue.length} remaining`);
      } else {
        node.log(`[UNS PUBLISH] Buffer flushed: ${successCount} sent, ${failCount} failed, ${messageQueue.length} remaining (${formatBytesToMB(remainingSize)} MB)`);
      }
      ...
      if (successCount > 0 && messageQueue.length === 0 && bufferPersistence !== 'none') {
        ...
        if (isDebug) {
          node.log(`[UNS PUBLISH] Cleared persisted buffer after successful flush`);
        }
      }
    ...
    const publishBatch = async () => {
      ...
      if (isDebug) {
        node.log(`[UNS PUBLISH] Publishing batch: ${batchCount} messages`);
      }
      ...
      const publishPromises = batchCopy.map(batchedMsg => 
        processPublish(batchedMsg)
          ...
          .catch(err => {
            node.error(`[UNS PUBLISH] Error publishing batched message: ${err.message}`, batchedMsg);
      ...
      if (isDebug) {
        node.log(`[UNS PUBLISH] Batch published: ${successCount} sent, ${failCount} failed (total: ${batchedMessagesCount})`);
      }
    ...
    const addToBatch = (msg) => {
      ...
      if (isDebug) {
        node.log(`[UNS PUBLISH] Message added to batch (${batchQueue.length}/${batchSize})`);
      }
    ...
    node.on('input', async function (msg) {
      try {
        ...
        if (enableAutoReply && !msg._autoReplyProcessed) {
          ...
          if (isDebug) {
            node.log(`[UNS PUBLISH] Auto-reply: Waiting for response message...`);
          }
        ...
        if (this.config.connectionStatus !== 'connected') {
          ...
          if (msg._flushing || msg._batched) {
            node.warn(`[UNS PUBLISH] Processing failed - connection lost`);
            return;
          }
          ...
          if (enableBuffer) {
            ...
            if (buffered && isDebug) {
              if (bufferSizeType === 'count') {
                node.log(`[UNS PUBLISH] Message buffered (${messageQueue.length}/${bufferSize} messages)`);
              } else {
                const currentBufferSize = calculateBufferSize();
                node.log(`[UNS PUBLISH] Message buffered (${formatBytesToMB(currentBufferSize)}/${formatBytesToMB(bufferSizeBytes)} MB)`);
              }
            }
          ...
          if (!canSendMessage()) {
            ...
            switch (rateLimitAction) {
              case 'drop':
                // Silent drop
                if (isDebug) {
                  node.log(`[UNS PUBLISH] Rate limit exceeded, message dropped (total: ${droppedByRateLimit})`);
                }
                return;
                
              case 'drop-warn':
                // Drop with warning
                node.warn(`[UNS PUBLISH] Rate limit exceeded (${rateLimit}/${rateLimitWindow}ms), message dropped (total: ${droppedByRateLimit})`);
                return;
              ...
              case 'delay':
                ...
                delayQueue.push(msg);
                if (isDebug) {
                  node.log(`[UNS PUBLISH] Rate limit exceeded, message queued for delay (queue: ${delayQueue.length})`);
                }
                // Don't override batch status if batch is enabled
                if (!enableBatch) {
                  node.status({ 
        ...
        switch (config.dataformat) {
          case 'uns_value':
            ...
            // Debug logging for UNS Value (only if debug enabled)
            if (isDebug) {
              node.log(`[UNS PUBLISH] Input payload: ${msg.payload} (type: ${typeof msg.payload})`);
            }
            ...
            if (datatypeOverride !== 'auto') {
              ...
              // Convert value to string for non-object types (UNS spec requirement)
              if (message.datatype !== 6) {
                message.value = String(message.value);
              }
              
              if (isDebug) {
                node.log(`[UNS PUBLISH] Manual datatype override: ${message.datatype} (validated)`);
              }
            } else {
              ...
              switch (typeof message.value) {
                case 'string':
                  message.datatype = 4;
                  if (isDebug) node.log(`[UNS PUBLISH] Detected string, datatype: 4`);
                  break;
                case 'number':
                  ...
                  if (Number.isInteger(message.value) && 
                    ...
                    if (isDebug) node.log(`[UNS PUBLISH] Detected Unix timestamp: ${message.value}, datatype: 5`);
                  } else if (Number.isInteger(message.value)) {
                    message.datatype = 1; // Integer
                    if (isDebug) node.log(`[UNS PUBLISH] Detected integer: ${message.value}, datatype: 1`);
                  } else {
                    message.datatype = 2; // Float
                    if (isDebug) node.log(`[UNS PUBLISH] Detected float: ${message.value}, datatype: 2`);
                  }
                  message.value = String(message.value);
                  break;
                case 'boolean':
                  message.datatype = 3;
                  if (isDebug) node.log(`[UNS PUBLISH] Detected boolean: ${message.value}, datatype: 3`);
                  message.value = String(message.value);
                  break;
                case 'object':
                  message.datatype = 6;
                  ...
                  break;
                default:
                  if (isDebug) node.log(`[UNS PUBLISH] Unknown type: ${typeof message.value}`);
                  return;
              }
            }
            ...
            if (isDebug) {
              node.log(`[UNS PUBLISH] Final message: ${message}`);
              node.log(`[UNS PUBLISH] Subject: uns.${config.datapointid}`);
            }
          ...
          case 'event':
            ...
            if (isDebug) {
              node.log(`[UNS PUBLISH] Input event type: ${msg.type || 'point'}`);
            }
            ...
            if (msg.id) {
                ...
                if (eventType === 'interval_start') {
                    ...
                    if (isDebug) node.log(`[UNS PUBLISH] Generated UUID for event: ${message.id}`);
                }
            }
            ...
            if (isDebug) {
              node.log(`[UNS PUBLISH] Final event message: ${message}`);
              node.log(`[UNS PUBLISH] Subject: event.${config.datapointid}`);
            }
          ...
          case 'specific_topic':
            // For specific topic: Use custom subject and send payload as-is (JSON stringified)
            subject = config.datapointid; // Use datapointid as the complete subject
            
            // Payload can be anything: object, string, number, etc.
            ...
            if (isDebug) {
              node.log(`[UNS PUBLISH] Specific topic payload: ${message}`);
              node.log(`[UNS PUBLISH] Subject: ${subject}`);
            }
            break;
        }
        ...
        if (config.debug) {
          node.log(`[UNS PUBLISH] Publishing to NATS: ${subject}`);
        }
        
        natsnc.publish(subject, message, function (err, guid) {
          if (err) {
            const cleanError = {
              message: err.message,
              code: err.code,
            ...
            if (config.debug) {
              node.log(`[UNS PUBLISH] Successfully published with GUID: ${guid}`);
            }
          }
        });
      } catch (err) {
    ...
    node.on('close', function () {
      ...
      if (enableBuffer && bufferPersistence !== 'none' && messageQueue.length > 0) {
        ...
        if (isDebug) {
          node.log(`[UNS PUBLISH] Saved ${messageQueue.length} messages before closing`);
        }
      }
      
      if (isDebug) {
        node.log(`[UNS PUBLISH] Node closed, cleaned up timers and saved buffer`);
      }
    });
  ...
  RED.nodes.registerType('uns-publish', NatsPublishNode);
};
